...
MODULE output_man
   ...
   use monitor, only: dimension_monitor, monitor_defined, monitor_dt
   use monitor, only: monitor_time, monitor_next_time
   use monitor, only: monitor_tavg_dt, monitor_next_tavg_time
   use monitor, only: monitor_tavg_ss_tol, monitor_ss, monitor_ss_exit
...
! Number of monitors that are checking for steady state
      INTEGER :: monitor_ss_count
...
! Only regular (not debug) files are written (second argument is zero)
      ...
      IF(any(MONITOR_DEFINED)) THEN
         ...
         ! Compute time-average (do not write data)
            IF(CHECK_TIME(MONITOR_NEXT_TAVG_TIME(LC))) THEN
               MONITOR_NEXT_TAVG_TIME(LC) = NEXT_TIME(MONITOR_TAVG_DT(LC))
               CALL WRITE_MONITOR(LC, .true.) ! This call will only compute the average, not save the data in the .csv file
            ENDIF

! Get instantaneous data + write data
...
! All monitors having non-zero monitor_tavg_ss_tol must have reached
! steady state to exit MFiX
         monitor_ss_exit = .true.
         monitor_ss_count = 0
         DO LC = 1, DIMENSION_MONITOR
            IF( monitor_tavg_ss_tol(LC)>ZERO) THEN
               monitor_ss_count = monitor_ss_count + 1
               IF(.NOT.monitor_ss(LC)) monitor_ss_exit = .false.
            ENDIF
         ENDDO
         IF(monitor_ss_count == 0) monitor_ss_exit = .false.
      ENDIF
...
! Initialize MONITOR_TIME
      DO LC = 1, DIMENSION_MONITOR
         ...
         MONITOR_NEXT_TIME(LC) = UNDEFINED
         IF (MONITOR_DT(LC) /= UNDEFINED) THEN
            IF (RUN_TYPE == 'NEW'.OR.RUN_TYPE=='RESTART_2') THEN
               MONITOR_TIME(LC) = TIME
               MONITOR_NEXT_TIME(LC) = TIME
            ELSE
               MONITOR_TIME(LC) = TIME
               MONITOR_NEXT_TIME(LC) = MONITOR_DT(LC) *                     &
