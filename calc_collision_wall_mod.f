...
!$omp    coh_force_previous_facet, normtest,bc_id,wall_tang_vel, &
!$omp    etan_des_w,etat_des_w,fnorm,overlap_t,ftan,ftmd,fnmd,pos_tmp, &
!$omp    mew_rw,rftow, omega_new,omega_mag)                            &
!$omp shared(max_pip,focus_particle,debug_des,                         &
!$omp    pijk,dg_pijk,des_pos_new,    &
!$omp    des_radius,facets_at_dg,vertex,bc_id_stl_face,  &
...
!$omp    des_etan_wall,des_etat_wall,dtsolid,fc,norm_face,time,             &
...
! Get BC_ID
            ...
            if(bc_id>0) then
               if(bc_id==3) then
! Get specific tangential wall for each dynamic _bc
                  wall_tang_vel(1) = zero
                  wall_tang_vel(2) = 2.0D0*Pi*5.0D0*4.5D0/1000.0D0*cos(2.0D0*Pi*5.0D0*time)
!                  wall_tang_vel(2) = zero
                  wall_tang_vel(3) = zero
               else if(bc_id==4) then
                  wall_tang_vel(1) = zero
                  wall_tang_vel(2) = 2.0D0*Pi*5.0D0*4.5D0/1000.0D0*cos(2.0D0*Pi*5.0D0*time)
!                  wall_tang_vel(2) = zero
                  wall_tang_vel(3) = zero
               ...
               call cfrelvel_moving_wall(ll, v_rel_trans_norm, vrel_t, &
                    normal, distmod, wall_tang_vel)
            else
...
!----------------------------------------------------------------------!
      SUBROUTINE CFRELVEL_MOVING_WALL(LL, VRN, VRT, NORM, DIST, wall_tang_vel)
...
! Wall translation velocity at contact point
      DOUBLE PRECISION, DIMENSION(DIMN), INTENT(IN) :: WALL_TANG_VEL
...
! Total relative velocity + rotational contribution
      ...
      VRELTRANS(:) =  DES_VEL_NEW(LL,:) + DES_CROSSPRDCT(V_ROT, NORM) - wall_tang_vel
