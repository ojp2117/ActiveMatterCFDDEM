...
!-----------------------------------------------
      integer :: lfaces
      integer :: lmaxlen1,lmaxlen2,lmaxarea,ii
      integer :: lmaxDGPacket
...
!-----------------------------------------------
      ...
      IF(MPPIC) THEN
         ...
         iParticlePacketSize = 20 + des_usr_var_size

         if(energy_eq) iParticlePacketSize = &
              iParticlePacketSize + dimension_n_s

         if(des_explicitly_coupled) iParticlePacketSize = &
              iParticlePacketSize + 3

         ! Random value. This gets resized as needed.
         lmaxDGPacket = 50*iParticlePacketSize
...
!**************************************qg***************************************************              
         ...
         lmaxDGPacket = 100*iGhostPacketSize

      ENDIF

! JFD: Need to also increase Ghost Packet size (see mpi_pack_des_mod.f and ! mpi_unpack_des_mod.f)
      IF(CGDEM) THEN
         iParticlePacketSize = iParticlePacketSize + 2
         iGhostPacketSize    = iGhostPacketSize    + 2
      ENDIF

      IF(SAVE_PART_RRATES)  THEN
         iParticlePacketSize = iParticlePacketSize + NO_OF_DES_RXNS
         iGhostPacketSize    = iGhostPacketSize    + NO_OF_DES_RXNS
      ENDIF
...
! not extend the domain for periodic walls because 1) they should not
...
! Comments         : In the main thread (pe_io) particles will be separated
...
! set the packet size for transfer, pos/vel + density/radius + statwt + real_particle_radius (if CGDEM)
      lpacketsize = 2*rdimn + 2
      if(CGDEM) lpacketsize = lpacketsize + 2
...
! JFD: moved 2 lines within single if statement
            if(CGDEM) then
               drootbuf(lbuf) = dpar_statwt(lcurpar); lbuf = lbuf + 1
               drootbuf(lbuf) = dpar_realpr(lcurpar); lbuf = lbuf + 1
            endif
...
! JFD: moved 2 lines within single if statement
         if(CGDEM) then
            des_cgp_stw(lcurpar) = dprocbuf(lbuf); lbuf = lbuf+1
            des_cgp_rpr(lcurpar) = dprocbuf(lbuf); lbuf = lbuf+1
         endif
